...
- **⚠️ 每次推送前记得**：①水印 +1 ②pubspec version minor+1 且 +N（versionCode）+1（当前 1.7.0+9 / 水印 b0703-13 / DB v17）。APK 下载链接：`.../android-latest/肥喵记账.apk`。
- **智能分类方案进度**（用户 md 复盘后的四层）：地基(商户分流+商品优先+置信度+按商户学习)已做(b0703-11)；支付宝退款归零已做(b0703-13)。**还没做**：①第3层导入复核页(按商户分组批量归类+喂学习)②第4层一次批量AI兜底(去重opt-in)③AI记账侧增量(用户分类树+学习项塞进DeepSeek提示词)④Phase A 分类大改(GPT `记账分类最终版调整建议.md`：医疗/教育补子类、收入细分、居家生活→居家住房、理财→投资收益、新增车辆支出/保险保障一级；新分类用 emoji 图标即"同款"，不用画SVG。**缓做**：特殊交易类型 还款/借贷/投资本金 那套复式记账，太重、和可爱简单路线有张力，现有转账+不计入收支已覆盖80%)。
