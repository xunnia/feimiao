...
主对话由 Fable 5 担任编排者：负责理解需求、做方案决策、写关键/精巧代码、审查子代理产出、向用户汇报。**凡是会产生大量 token 消耗的跑腿与重活，必须派给对应档位的子代理**，保持主对话上下文干净：

| 任务类型 | 派给 | 模型 |
|---|---|---|
| 代码库检索、找文件/符号、翻日志、简单事实核查 | `haiku-scout` | haiku |
| 按明确规格写代码、批量重构、写测试、修 lint/CI 小错、写文档 | `sonnet-builder` | sonnet |
| 疑难 bug 根因分析、架构权衡、安全/性能审计、反复失败的 CI 诊断 | `opus-reasoner` | opus |
| 需求决策、最终审查、对用户的总结汇报、一次性小修改 | Fable 主对话 | fable |
