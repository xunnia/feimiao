...
> 版本：v1，2026-08-27
>
> 目标：在不破坏 Android 现有开发的前提下，把肥喵记账迁移为原生 SwiftUI iOS App。业务结果与 Android 对齐；导航、材质、动效和系统集成按 iOS 规范重新设计。
...
Android 和 iOS 不追求逐像素复制。以下内容必须一致：
...
“iOS 有更好的原生效果”可以使用，但不能让玻璃、弹性动画或系统按钮改变账务结果和操作反馈。低性能设备或开启减少动态效果时要有降级路径。
...
- 资金/物品分栏和账户详情；
- 物品新增、历史补录、从账单加入、新购买（iOS 当前先完成档案、估值和生命周期基础）；
