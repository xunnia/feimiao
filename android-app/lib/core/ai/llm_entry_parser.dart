...
- 商品名/店铺名作为分类依据，并写进备注。
- 若是订单列表（多个商品/多个店铺）：**每个订单各记一笔**；订单有几个就记几笔，不要合并。
- **文本若含 ───── 分隔线**：这是按截图版面切好的「订单分块」。**金额只跟它同一块内、紧邻上方的商品配对，绝不跨块去取别的商品的金额。** 通常一块=一笔；若某块只有金额没有商品名（或只有商品名没有金额），就把它和紧邻的那块合并成同一笔。
- **关键(防错位)**：每个商品的付款金额 = 紧跟在该商品名后面的"自动确认收货并付款 ¥X"(或它自己那块里实付>0的数)。**逐个商品对应它自己下方的金额，绝不能用上一个/下一个商品的金额。**
  例：OCR 文本顺序是「舒肤佳沐浴露 … ¥35.6 … 自动确认收货并付款 ¥32.04 … 维达抽纸 … ¥22.5 … 自动确认收货并付款 ¥15.9」，
  必须记成：舒肤佳沐浴露=32.04、维达抽纸=15.9。**不要**记成维达=32.04 或舒肤佳=15.9。
