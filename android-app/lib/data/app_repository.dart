...
class AppRepository extends ChangeNotifier {
  ...
  Future<void> _initInternal({required bool fastStartup}) async {
    ...
    try {
      ...
      // Do not let an old credential read overwrite a new initialization.
  ...
  Future<bool> restoreBackupPackage(
    ...
    try {
      ...
      try {
        package = await compute(BackupPackageCodec.decodeFileRequest,
            [srcPath, p.join(work.path, 'decoded')]);
      } catch (_) {
        return false;
      }
      final databaseFile = package.diskFiles['database/$_dbName'];
      if (databaseFile == null) return false;
      final dbPath = p.join(work.path, _dbName);
      await File(databaseFile).copy(dbPath);
      ...
      for (final entry in package.diskFiles.entries) {
  ...
  Future<void> _normalizeStandaloneRefunds() async {
    final db = _db;
    if (db == null) return;
    // The common case needs no repair: avoid materializing the full ledger.
    final pending = await db.query('transactions',
        columns: ['id'],
        where: 'kind = ? AND refund_of IS NULL AND CAST(amount AS REAL) < 0',
        whereArgs: [TransactionKind.expense.toJson()],
        limit: 1);
