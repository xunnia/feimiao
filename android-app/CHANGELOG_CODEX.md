## 2026-09-12 v1.293.0+308：修复启动优化的完整性边界（已发布 VPS）

- 首屏前恢复分类图标设置，并完成退款归并、到期预算和周期账处理；普通无待归并退款情况不再加载整表，历史/资产仍延迟加载。
- 初始化失败重试重新接回后续加载和后台服务；服务注册按仓库实例去重，连续点重试不会重复启动；AI 不在历史加载失败时继续发起查询。
- 外部分享队列在初始化失败时保留并暂停，恢复后唤醒；仅进程内保留，不宣称跨进程持久化。
- 公共页面路由和首页历史月份增加数据加载保护，月份选择等待完整历史，不把半快照当空账本。
- 修复批全量 1221/1221 通过；发布前定向再测 12/12、发布规则 9/9，通过 Release 构建与固定证书/V2/16 KiB 身份门禁。
- APK `ci-artifacts/releases/feimiao-codex-v1.293.0-308.apk`，117936705 字节，SHA256 `73DF20B0BA3C1A3AF1CFE5F0F9EF7CC0F0A38C5DF155239493A54CF3DCDC213E`。
- VPS `v308-73df20b0ba3c` 发布成功，新旧更新入口一致，Range 206 片段与本地一致。保留 306/307/308，线上 305 及回退条目已移除，本地 305 原包保留；回退目录目前为空。
- 手机安装/冷启动、加载态实机对比图和公网整包重新下载哈希未验收；详细边界见 `docs/STARTUP_INTEGRITY_2026-09-12.md`。

## 2026-09-11 v1.292.0+307：恢复主页滑动抽屉（已发布 VPS）

- 根因：RootShell 的关闭态快速返回分支跳过了 Pointer Listener，首次进入主页无法滑动打开；开关时还改变主页面元素树位置。
- 修复：关闭/拖动/打开保持同一元素树与手势监听，保留抽屉惰性加载；关闭状态禁用裁剪，阴影仍按展开进度启用。方向保持从左侧向右拖开、向左拖关闭。
- 定向手势及启动测试 5/5 通过：首次滑动、关闭重开、主页元素状态保留、纵向/反向拖动不误开、账单操作面板打开时让位。
- Flutter 全量 1217/1217、发布规则 9/9；本轮修改文件静态分析无问题，Release 构建及包名/版本/16 KiB/V2/固定证书门禁通过。
- APK：`ci-artifacts/releases/feimiao-codex-v1.292.0-307.apk`，117920321 字节，SHA256 `CDAE672E13DB58E1926781293216D2007B1CF69D2613790EBA8591DE2CED2DAF`。
- 用户授权后发布 VPS `v307-cdae672e13db` 成功；新旧公网版本入口一致，Range 206 的前 1024 字节与本地包一致；服务端校验完整包哈希。保留 305/306/307，清理线上 304，本地 304 保留。
- 未完成真实设备安装、冷启动、手势观感或性能验收；未完成公网整包重新下载哈希复验。日志 `.tmp/drawer-307-tests.log`、`.tmp/drawer-307-build.log`。

## 2026-09-10 v1.291.0+306 代码复核修复（本地 APK，待安装验收）

- 更新校验：空/非法 SHA256 不再放行；元数据及下载响应头都缺哈希时拒绝完成；新客户端直连 VPS 更新和回退目录。
- 延迟初始化：失败后释放 Future，可再次尝试；成功清除错误，维护标记只在对应步骤成功后清除。
- V2 AI 托管请求：按任务隔离网络资源，取消/完成关闭所属连接；不关闭其他任务共享连接，不承诺上游一定停止计费。
- 备份恢复：后台 isolate 从磁盘读 ZIP，逐文件校验并落盘，保留原恢复回滚；增加声明体积、条目数和不安全路径防护，避免全部附件字节同时驻留。
- 验证：定向 30/30、Flutter 全量 1216/1216、发布身份规则 9/9；analyze exit 0（90 条 warning/info，无 error）。Release 构建、16 KiB 对齐、V2 和固定证书核对通过；无在线 ADB，不能声称安装/冷启动已通过。
- APK：`ci-artifacts/releases/feimiao-codex-v1.291.0-306.apk`；SHA256 `9F53F8216BA8671C2DED45A73CB6A09B51EF253B08DEC951AE9B8A2CD8B658A6`。
- 安装序号 306 避开 VPS 专用回退包占用的 305；用户随后授权上线，VPS 发布 `v306-9f53f8216ba8` 成功，新旧版本入口一致，保留 304/305/306 三包。公网整包下载哈希复验未完成，手机安装与冷启动仍未验收。性能/架构剩余项见 `docs/CODE_AUDIT_2026-09-10.md`。

## 2026-09-01 v1.289.0+304 Android 回退安装链路与启动修复

- **普通安装器边界已固化**：Android 仍要求同包名、同签名且安装 `versionCode` 不低于当前包；`REQUEST_INSTALL_PACKAGES` 不能授予降级权限。新增历史版本目录、兼容包重打包脚本和回退安装序号规则，原始低版本 APK 不再被误当作可回退包。
- **更新检查修复**：原生 `PackageManager` 返回实际安装序号，兼容回退包内嵌旧 Flutter 版本号时不会错误提示更新或清理错误的下载记录。
- **历史版本入口**：设置页新增“历史版本”，只允许选择安装序号高于当前包的、签名校验过的条目；下载、SHA-256 校验和 FileProvider 安装复用现有链路。
- **线上目录与保留**：Worker 新增 `/rollback.json`；发布清理会保留目录明确引用的本 KV 历史分片，外部 HTTPS 归档不占 KV。默认热路径仍只保留当前版本和上一版本。
- **构建序列**：当前修复版升级为 `1.289.0+304`；从 `1.279.0` 生成的下一兼容包应使用安装序号 `305`。数据库兼容仍需逐版本 fixture 门禁，不能只因安装成功就宣称数据安全。

## 2026-09-01 v1.288.0+303 Android 真机秒闪退回归修复

- **真机反馈根因范围**：`v1.287.0+301` 虽通过 Flutter 测试和 Release 门禁，但生产入口改为“仓库未完成时先挂载 MaterialApp/启动 gate”；目标手机实际冷启动仍立即退出，说明原测试没有覆盖真实 Android 启动时序。
- **启动顺序回归**：恢复已在用户手机可正常运行的 `v1.279.0` 启动顺序——系统代理查询有界完成、仓库主页快照 ready 后才挂载完整页面；数据库初始化失败仍进入恢复页，后台报告/OAuth/小组件继续留在首帧之后。
- **升级边界**：versionCode 直接升到 `303`，可覆盖用户正在使用的 `v1.279.0` 回滚兼容包（安装层 versionCode=302），不需要卸载且保留账本数据。
- **门禁要求**：除全量 Flutter/analyze/Release/签名/16 KiB 外，必须实际安装 Android 设备并完成冷启动、强停后重启和数据保留检查；未完成不得标记交付。

## 2026-09-01 v1.287.0+301 Android 启动闪退修复（真机未通过）

- **启动稳定性**：首帧前不再等待数据库、系统代理或平台通道；数据库初始化失败留在应用内恢复页，不再把异常传播成进程退出。
- **平台通道**：启动系统栏调用改回 Flutter 正确的同步 API，不再把返回 `void` 的调用包装成 `unawaited` Future；系统代理和延迟服务继续隔离在首帧之后。
- **验证**：Flutter 全量回归 `1203/1203`；Dart analyze 0 error（91 条既有 warning/info）；Release 包名/版本、16 KiB 对齐、APK V2 与固定证书门禁通过。
- **APK**：`C:\src\xunni-codex\ci-artifacts\releases\feimiao-codex-v1.287.0-301.apk`，117,674,561 字节，SHA256 `D925E1925AA051C0E07CDC02F55BC3D7E7893A56AE99073001BDA8DCB04002FB`。
- **验证边界**：当时没有在线 Android ADB；随后用户真机确认安装后仍秒闪退，因此该版本不作为可用交付版本。

## 2026-09-01 v1.286.0+300 Android 冷启动闪退防护

- **启动边界**：系统代理读取增加 3 秒上限和异常兜底；数据库 ready 等待增加 8 秒上限。代理或数据库异常不会阻塞 Flutter 首帧，主页先安全绘制并在后台继续恢复。
- **Android 网络配置**：恢复“默认禁止明文、仅 localhost 回调允许 HTTP”的安全配置；HTTPS 服务商请求不受影响，避免最近全局明文配置在部分 ROM 上触发启动兼容问题。
- **验证**：启动回归 `4/4` 通过；完整 Flutter、Dart analyze、Kotlin 编译及 Release APK 门禁待本轮构建完成；当前无在线 Android ADB，真机安装冷启动仍需目标设备复测。
...
## 2026-09-02 v1.290.0+305 Android 首屏启动收口

- **首屏口径**：启动完成标记只在首页真实账本快照完成绘制后触发，包含月份、收支汇总、结余、预算、筛选和当月账单列表，不再用启动壳或空页面冒充首屏完成。
- **关键路径缩短**：账本列表与当前账本选择并行读取；首页账单使用稳定只读引用，避免重建时复制整张可见账单表；已完成的运行时数据库兼容标记默认跳过重复结构探测。
- **后台解耦**：系统代理发现不再阻塞首页或后台数据库收敛；聊天目录、AI 健康、记忆、报告计划和工具开关延后到首帧后加载，聊天发送会在请求前等待完整收敛，避免首条消息误用默认配置。
- **验证**：启动/仓库/性能定向测试 `6/6`；Dart analyze `0 error`（90 条既有 warning/info）；Release 包需继续通过安装、冷启动、首页截图、签名和 16 KiB 门禁。
