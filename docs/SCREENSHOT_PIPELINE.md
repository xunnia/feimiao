...
运行 `34684572862`：iOS 截图 job 通过，Android 批次 0/1/2/3 通过。批次 4 在首场景 transactions-search 安装 APK 后连接 VM service 超时，尚未进入测试；没有图片产物，因此汇总正确阻断。后续候选改为 Flutter 官方支持的 `--no-dds` 直连 VM service，并在清理前采集转发、进程与日志信息；不增加重试次数、不把超时当成功。此修改仍需 CI 验证，DDS 不能仅凭连接超时确认为唯一根因。
